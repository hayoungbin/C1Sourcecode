...
게임 내의 사운드 출력은 SoundManager.cs에서 구현된 기능들을 통해 이루어집니다.
...
이번 프로젝트의 용량이 조금 커서 시작화면에서 스타트 버튼을 누르면 로딩이 끝날 때 까지 시작화면에서 멈추는 일이 있었습니다.

스타트버튼이 눌러졌는지 아닌지도 분간이 되지 않고 영 보기 좋지 않았기에 로딩화면을 만들었습니다.
